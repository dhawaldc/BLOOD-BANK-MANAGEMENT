...
public:
    Donor(string name, long int rollnum, string phone_no, string email, string aadhar_no, string blood_group, string dept, int num_of_blood, string lst_dt_of_bd)
        : Roll(rollnum), User(phone_no, email, aadhar_no), Name(name), Department(dept), Blood_group(blood_group) {
...
int main() {
    ...
    while (num--) {
        ...
        long int rollnum;
        int num_of_blood;
        infile >> phonenum >> email >> addhar_no >> blood_group >> dept >> rollnum >> num_of_blood >> lst_dt_of_bd;
